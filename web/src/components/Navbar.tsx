  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between px-[2rem] h-[56px] bg-[#F9F9F9]/90 backdrop-blur-[12px] border-b border-[#e2e8f0]">
      ...
      <div className="hidden md:flex items-center gap-[2.25rem]">
        ...
        <Link className={`text-[0.82rem] no-underline transition-colors duration-200 flex flex-col items-center relative font-medium ${isPricingActive ? 'text-[#5B54F7]' : 'text-[#475569] hover:text-[#0f172a]'}`} to="/pricing">
          Pricing
          {isPricingActive && (
      ...
      <Link className="inline-flex items-center justify-center py-[0.6rem] px-[1.25rem] bg-[#5B54F7] text-white rounded-full text-[0.82rem] font-medium no-underline border-none cursor-pointer transition-all duration-200 hover:bg-[#4a43e6] hover:-translate-y-[1px] tracking-[0.01em]" to="/pricing">
        Claim My Link
      </Link>
    </nav>
  )
